...
export interface OpenAICompatClientOptions {
  baseUrl: string;
  apiKey?: string;
  // The literal value sent in `body.model`. When omitted, `req.model` is forwarded
  // (matches the public OpenAI behaviour). LocalClient sets this to the user-supplied
  // tag (`qwen2.5-vl:7b`) while `req.model` stays `'local'` so `getModel('local')`
  // still resolves the runtime cache for cap / pricing.
  apiModelName?: string;
...
export class OpenAICompatClient implements AgentClient {
  private readonly baseUrl: string;
  private readonly apiKey: string;
  private readonly apiModelName: string | null;
  ...
  constructor(opts: OpenAICompatClientOptions) {
    ...
    this.apiModelName = opts.apiModelName ?? null;
    this.providerLabel = opts.providerLabel ?? 'openai-compat';
  ...
  async sendTurn(req: AgentTurnRequest, opts?: SendOptions): Promise<AssistantTurnResult> {
    ...
    const wireModel = this.apiModelName ?? req.model;
    for (;;) {
      ...
      if (budget >= cap) {
        throw new TruncationError(this.providerLabel, cap, result.requestId);
      }
  ...
  private async fetchWithRetry(body: unknown, signal?: AbortSignal): Promise<Response> {
    ...
    while (attempt <= this.maxRetries) {
      try {
        ...
        await handleErrorResponse(res, this.providerLabel);
      } catch (err) {
        lastErr = err;
    ...
    throw lastErr instanceof Error ? lastErr : new NetworkError(this.providerLabel);
  }
}
...
async function handleErrorResponse(res: Response, providerLabel: string): Promise<never> {
  ...
  if (res.status === 401 || res.status === 403) {
    throw new AuthError(providerLabel, extractMessage(bodyText) ?? 'Unauthorized.', requestId);
  }
  if (res.status === 429) {
    const retryAfter = parseRetryAfterMs(res.headers.get('retry-after'));
    throw new ProviderRateLimitError(providerLabel, retryAfter, requestId);
  }
  if (res.status >= 500 && res.status < 600) {
    throw new NetworkError(providerLabel, `HTTP ${res.status}: ${bodyText.slice(0, 200)}`, requestId);
  }
  if (res.status === 400) {
    const msg = extractMessage(bodyText) ?? `HTTP 400: ${bodyText.slice(0, 200)}`;
    throw new NetworkError(providerLabel, msg, requestId);
  }
  throw new NetworkError(providerLabel, `HTTP ${res.status}: ${bodyText.slice(0, 200)}`, requestId);
}
