...
/**
 * Compute the amount each member owes for this expense, in the expense's own
 * currency minor units. Invariant: Σ of returned values === expense.amountMinor
 * whenever the split is valid.
 *
 * Members not in `expense.split` are omitted from the returned map.
 */
